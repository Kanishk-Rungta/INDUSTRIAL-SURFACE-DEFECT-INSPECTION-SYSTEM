...
(function () {
  ...
  var deviceSelect = document.getElementById("camera-device");
  var preview = document.getElementById("capture-preview");
  var inspectBtn = document.getElementById("capture-inspect");
  var removeBtn = document.getElementById("capture-remove");
  var retakeBtn = document.getElementById("capture-retake");
  ...
  function startCamera() {
    ...
    var selectedDevice = deviceSelect && deviceSelect.value;
    navigator.mediaDevices.getUserMedia({
      video: selectedDevice
        ? { deviceId: { exact: selectedDevice }, width: { ideal: 1280 }, height: { ideal: 960 } }
      ...
      navigator.mediaDevices.enumerateDevices().then(function (devices) {
        var current = deviceSelect.value;
  ...
  function chooseFile(file) {
    ...
    pendingFile = file;
    showPreview(file);
    setStatus("Image ready. Review the preview, then press Inspect image.");
  }
  ...
  function clearPreview() {
    ...
    inspectBtn.disabled = true; removeBtn.disabled = true; retakeBtn.disabled = true;
  }
  ...
  function submit(blob, filename, source) {
    ...
    form.append("station_id", root.getAttribute("data-station") || "line-1-cam-A");
    var batchId = document.getElementById("capture-batch").value.trim();
    ...
    fetch("/api/inspections", { method: "POST", body: form })
  ...
  function render(payload) {
    ...
    var original = document.getElementById("capture-original");
    ...
    if (inspection.source_image_url) {
      original.src = inspection.source_image_url;
  ...
  deviceSelect.addEventListener("change", function () { if (stream) { stopCamera(); startCamera(); } });
  document.querySelectorAll("[data-view]").forEach(function (button) {
    button.addEventListener("click", function () {
      var view = button.getAttribute("data-view");
      document.getElementById("capture-original").hidden = view === "overlay";
      document.getElementById("capture-image").hidden = view === "original";
    });
  });
