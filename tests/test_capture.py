...
def post_common(client, payload: bytes, *, source_type: str = "upload", mime: str = "image/png"):
    return client.post(
        "/api/inspections",
        files={"image": ("frame.png", payload, mime)},
        data={"source_type": source_type, "material": "steel", "product_id": "common/item-1"},
    )


def test_common_endpoint_accepts_upload_and_camera_through_one_handler(client):
    upload = post_common(client, png_bytes(), source_type="upload")
    camera = post_common(client, png_bytes(), source_type="camera")
    assert upload.status_code == camera.status_code == 200
    assert upload.json()["source_type"] == "upload"
    assert camera.json()["source_type"] == "camera"
    assert upload.json()["provider"] == camera.json()["provider"] == "mock"


def test_common_endpoint_rejects_unsupported_mime_and_magic_bytes(client):
    wrong_mime = post_common(client, png_bytes(), mime="text/plain")
    wrong_magic = post_common(client, b"not an image")
    assert wrong_mime.status_code == 415
    assert wrong_magic.status_code == 415


def test_common_endpoint_distinguishes_clean_from_failure(client):
    result = post_common(client, png_bytes()).json()
    assert result["empty"] is (result["status"] == "clean")
    assert result["status"] in {"clean", "regions_found"}
